...
import { depression } from '../inspectsArrays';

import Tnb from '../components/Tnb';
import Question from '../components/Question';
import AnswerYN from '../components/AnswerYN';

let transformQuestion = shuffle(depression);
let temp_info = '';


const Button = styled.button``;

const Depression = ({ history, modal }) => {
  const [startTime, setStartTime] = useState(setTime());
  const maxNbr = depression.length;
  ...
  if (Qnbr > maxNbr) {
    ...
    history.push('/end/depress/' + score + '/' + startTime); //검사, 점수, 시작 시간 넘겨 페이지 이동
  }

  return (
    <div
      className="inspects_wrap depression question">
      <h1 className="blind_inline">정신건강 테스트 앱</h1>
      <Tnb title="우울증" onoff={soundOnOff} onClickSpeaker={toggleSpeaker} onClickPrev={toggleModal}/>

      {(maxNbr >= Qnbr) && (
        <div className="contents_wrap">
          <Question ary={currentAry} type="depression" qnbr={Qnbr} soundState={soundOnOff}/>
